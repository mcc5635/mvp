    ...
    <Popover className="relative">
      <PopoverButton className="inline-flex gap-x-1 text-sm font-semibold leading-6 text-gray-900 bg-white p-2 min-w-60 justify-between text-left ">
        <div className=''>
            ...
            <p>{props.title}</p>
            <span><Icon icon={'mdi:info'} fontSize={18}/></span>
            </div>
      ...
      >
        <div className="w-screen max-w-md flex-auto overflow-hidden rounded-sm bg-white text-sm leading-6 shadow-lg ring-1 ring-gray-900/5">
          <div className='pt-2 px-2 font-bold text-left'>
          {props.title}
          </div>
          <div className="p-2">
            {solutions.map((item) => (
              <div key={item.name} className="group relative flex gap-x-6  p-4  rounded-sm border border-gray-300 my-1 hover:bg-[#4b3fa0]">
